...
popSize.ageG <- apply(pop_region, 1, sum)
...
neg_binom_log_lik_France <- function(R0,
  ...
  ICU_occ <- apply(mat_ICU_occ[,colnames(mat_ICU_occ) %in% vect_name_region], 1, sum)
  daily_hosp_admissions <- apply(mat_daily_hosp[,colnames(mat_daily_hosp) %in% vect_name_region], 1, sum)
  hosp_occ <- apply(mat_hosp_occ[,colnames(mat_hosp_occ) %in% vect_name_region], 1, sum)
...
calculateLL_neg_binom_France <- function(param){
  ...
  g_out_hosp <- param[3]
  alpha <- param[4]
  R0_after_intervention <- param[5]
  log_Inf_lockdown <- param[6]
  delta <- param[7]
...
if(run_MCMC){
  ...
  init_g_out_ICU <- runif(n = 1, min = 0, max = 1)
  init_g_out_hosp <- runif(n = 1, min = 0, max = 1)
  ...
  inf_prior <- c(0, 0, 0, 0, 0, log(1e2), 0)
  sup_prior <- c(25, 2, 2, 2,25, log(1e8), 2)
  ...
  ## (F) First iteration
  iteration <- 1
  logLik[iteration] <- calculateLL_neg_binom_France(param)
...
source('Scripts/functions_results_France.R')
...
## Parameter estimates
param_estim(list_res, list_res, list_res, list_res, BI)

## Running simulation from the posterior distribution
n_simul_1 <- 2
n_simul_2 <- 10
list_res_simul_France <- simul_from_posterior(list_res, BI, n_simul_1, n_simul_2, seed = 1)
tmp_pred_daily_ICU_adm <- list_res_simul_France$pred_daily_ICU_adm
tmp_pred_ICU_occ <- list_res_simul_France$pred_ICU_occ
tmp_pred_daily_hosp_adm <- list_res_simul_France$pred_daily_hosp_adm
tmp_pred_hosp_occ <- list_res_simul_France$pred_hosp_occ
tmp_pred_new_inf <- list_res_simul_France$pred_daily_new_infections
tmp_pred_cum_inf <- list_res_simul_France$pred_cum_infections

dir_results_simul <- paste0('Results/', format(date_file, '%Y%m%d'), '/Simul')
ifelse(dir.exists(dir_results_simul), '', dir.create(dir_results_simul))
name_res_simul <- paste0(dir_results_simul, '/simul_baseline_')
saveRDS(tmp_pred_daily_ICU_adm, paste0(name_res_simul, 'list_res_pred_ICU_adm.rds'))
saveRDS(tmp_pred_ICU_occ, paste0(name_res_simul, 'list_res_pred_ICU_occ.rds'))
saveRDS(tmp_pred_daily_hosp_adm, paste0(name_res_simul, 'list_res_pred_hosp_adm.rds'))
saveRDS(tmp_pred_hosp_occ, paste0(name_res_simul, 'list_res_pred_hosp_occ.rds'))
saveRDS(tmp_pred_new_inf, paste0(name_res_simul, 'list_res_pred_new_inf.rds'))
saveRDS(tmp_pred_cum_inf, paste0(name_res_simul, 'list_res_pred_cum_inf.rds'))

##################################
## 11) Plotting simulations results
##################################
par(mfrow = c(2,2))
panel_ICU_hosp_France(tmp_pred_daily_ICU_adm, tmp_pred_daily_hosp_adm, tmp_pred_ICU_occ, tmp_pred_hosp_occ)

par(mfrow = c(1,2))
plot_new_infections(tmp_pred_new_inf)
plot_proportion_infected(tmp_pred_cum_inf)
